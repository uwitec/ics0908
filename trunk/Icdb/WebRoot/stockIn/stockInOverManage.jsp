	...
	<head>
 	...
 	function goback(){	
			if(window.confirm("终止当前工作返回上一页？")){
				window.location.href="searchStockInOver.action";
			}
		}
	  ...
	  function callBack(data){
		 if(data!=null && data=="success"){
		 	 window.location.href="searchStockInOver.action?backurl="+window.location.href;
		 }
	  }
...
//		var parameter = new Array(stockIn,array);
	...
	<body class="stock_body">
		...
		<s:form action="saveStockIn.action" theme="simple"
			...
			<table cellpadding="0" cellspacing="0" width="850" border="0" class="stock_table" cellpadding="0" cellspacing="0" border="0">
				<tr class="stock_tr">
					...
					<s:if test="stock.stockInStateType==1">
						...
						<table id="addMatail"class="stock_table" cellspacing="1" width="850">
									...
									<s:else>
										...
										<td>
											<s:if
												...
												<s:textfield cssStyle="display:none" name="cargoSpaceCode"
													id="cargoSpaceCode_%{#stockInMateriel.index+1}"/>
												<s:textfield name="cargoSpaceName" size="5" id="cargoSpaceName_%{#stockInMateriel.index+1}" onclick="showCargoSpace(%{#stockInMateriel.index+1})"/>
											</s:if>
										</td>
										<td>
			...
			<s:if test="stock.checkResult == 1">
				<a href="#" onclick="complStockIn(1);"><img src="<%=base%>/images/sub1_btn_save.jpg" border="0" alt="保存"></a>
				<a href="#" onclick="complStockIn(3);"><img src="<%=base%>/images/finish.jpg" border="0" alt="完成"></a>
